...
program
  ...
  .addHelpText('after', '示例: dasheng translate-yaml ./en.yaml --target ./locales/');
...
program
  ...
  .action(async (file, options) => {
    const { translateYamlCommand } = await import('./commands/translate-yaml.js');
...
program
  ...
  .option('-s, --style <style>', '封面风格 (tech/minimal/gradient/illustration/business)', 'tech')
  .option('-r, --ratio <ratio>', '封面比例 (16:9/1:1/9:16/4:3)，与 --width/--height 互斥')
  ...
  .action(async (file, options) => {
    const { generateCoverCommand } = await import('./commands/generate-cover.js');
...
program
  ...
  .option('-s, --style <style>', '博客风格 (release/tutorial/news/deep-dive/story)')
