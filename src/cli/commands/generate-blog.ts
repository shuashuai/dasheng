...
export async function generateBlogCommand(
  ...
  try {
    ...
    // 收集输入信息
    let input: { rawContent?: string; topic?: string; keyPoints?: string[]; targetAudience?: string; style?: BlogStyle };
    ...
    if (file && existsSync(file)) {
      ...
      input = {
        topic: options.topic,
        keyPoints: options.points ? options.points.split(',').map(p => p.trim()) : undefined,
        targetAudience: options.audience,
        style: options.style as BlogStyle
      };
      
      if (!input.topic && !input.keyPoints) {
        console.error(chalk.red('❌ 请提供主题(-t)、要点(-p)或使用交互式模式(-i)'));
        console.log(chalk.gray('\n示例:'));
        console.log(chalk.cyan('  dasheng generate-blog -t "React 19 新特性" -p "并发渲染,Server Components,Actions"'));
    ...
    for (let i = 0; i < articles.length; i++) {
      ...
      // 保存文件
      const outputPath = options.output || `blog-post-${Date.now()}-${i + 1}.md`;
      const finalPath = count > 1 ? outputPath.replace('.md', `-${i + 1}.md`) : outputPath;
    ...
    if (error instanceof Error && error.message.includes('API Key')) {
      ...
      console.log(chalk.cyan('    $env:OPENAI_BASE_URL="https://api.deepseek.com/v1"'));
    }
 ...
 * 交互式输入
 */
async function interactiveInput(): Promise<{ topic?: string; keyPoints?: string[]; targetAudience?: string; style?: BlogStyle }> {
