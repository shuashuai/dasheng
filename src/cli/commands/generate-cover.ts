...
import {
  CoverGenerator,
  CoverGenerateOptions,
} from "../../core/services/cover-generator.js";
...
export async function generateCoverCommand(
  ...
  try {
    ...
    if (file && existsSync(file)) {
      ...
      console.log(
        chalk.cyan('  dasheng generate-cover -t "大圣 1.0 发布" --style tech'),
      );
      process.exit(1);
    }
    ...
    if (options.prompt) {
      console.log(
        `   提示词: ${chalk.magenta(options.prompt.slice(0, 50))}${options.prompt.length > 50 ? "..." : ""}`,
      );
    }
    ...
    console.log(
      `   图片尺寸: ${chalk.blue(`${result.width}x${result.height}`)}`,
    );
    console.log(
      `   设计风格: ${chalk.yellow(getStyleDisplayName(result.style))}`,
    );
    console.log();
