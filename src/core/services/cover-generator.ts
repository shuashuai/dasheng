...
export class CoverGenerator {
   ...
   * 从 Markdown 文件提取信息
   */
  extractFromMarkdown(filePath: string): {
    title: string;
    subtitle?: string;
    author?: string;
    date?: string;
    ...
    return {
      title:
        ...
        this.extractFirstLine(parsed.content),
   ...
   * 渐变风 - 多彩流体 + 动态效果
   */
  private async drawGradientStyle(
    options: CoverGenerateOptions,
  ): Promise<void> {
    const { width, height, title, subtitle, author, date } = options;
    const ctx = this.ctx!;
   ...
   * 插画风 - 手绘温暖 + 丰富元素
   */
  private async drawIllustrationStyle(
    options: CoverGenerateOptions,
  ): Promise<void> {
    const { width, height, title, subtitle, author, date } = options;
    const ctx = this.ctx!;
   ...
   * 商务风 - 深色专业 + 金色点缀
   */
  private async drawBusinessStyle(
    options: CoverGenerateOptions,
  ): Promise<void> {
    const { width, height, title, subtitle, author, date } = options;
    const ctx = this.ctx!;
    ...
    ctx.lineWidth = 1;
    for (let i = 0; i < 4; i++) {
      ctx.strokeRect(40 + i * 25, 40 + i * 20, 120 - i * 15, 80 - i * 10);
    }
