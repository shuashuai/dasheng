...
import type { BlogStyle, BlogInput, BlogOutput, ReleaseNotes } from '../../types/index.js';
...
export class BlogGenerator {
  ...
  parseReleaseNotes(content: string): ReleaseNotes {
    ...
    // 尝试提取版本号
    const versionMatch = content.match(/#?\s*[Vv]ersion\s+(\d+\.\d+\.?\d*)/) || 
    ...
    for (const section of sections) {
      ...
      if (lowerSection.includes('feature') || lowerSection.includes('新增') || lowerSection.includes('新功能')) {
        result.features = this.extractListItems(section);
      } else if (lowerSection.includes('improvement') || lowerSection.includes('优化') || lowerSection.includes('改进')) {
        result.improvements = this.extractListItems(section);
      } else if (lowerSection.includes('bug') || lowerSection.includes('修复')) {
        result.bugfixes = this.extractListItems(section);
      } else if (lowerSection.includes('breaking') || lowerSection.includes('破坏性')) {
        result.breakingChanges = this.extractListItems(section);
      }
    }

    // 如果没有提取到任何内容，将整个内容作为特性
    if (result.features.length === 0 && result.improvements.length === 0 && 
        result.bugfixes.length === 0 && result.breakingChanges.length === 0) {
   ...
   * 生成单篇文章
   */
  private async generateSingle(input: BlogInput, index: number): Promise<BlogOutput> {
   ...
   * 构建提示词
   */
  private buildPrompt(input: BlogInput, style: BlogStyle, variation: number): string {
